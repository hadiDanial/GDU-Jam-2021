...
public class HookController : MonoBehaviour
{
    ...
    [SerializeField] private GameObject hookCollider;
    ...
    private void Update()
    {
        if(forcePosition && state == HookState.Held)
        {
            targetTransform.position = holdPoint.position;
        }
    }
    ...
    internal void SetAim(Vector2 dir, bool useArrow = true)
    {
        ...
        if(useArrow)
        {
            ...
            arrow.transform.rotation = MathP.RotationFromDirection(-aimDirection);
            if (aimDirection.x < 0) hookHeadRenderer.flipY = true;
            else if (aimDirection.x > 0) hookHeadRenderer.flipY = false;
        }
        else
        {
    ...
    private void GrabObject(float time, Transform targetT, bool throwObject = false)
    {
        ...
        if (hit)
        {
            ThrowHook(time, targetT, throwObject, hit);
        }
        else Debug.Log("No hits");
    }

    private void ThrowHook(float time, Transform targetT, bool throwObject, RaycastHit2D hit)
    {
    ...
    internal void Backthrow()
    {
        if(state == HookState.Empty)
        {
            GrabObject(backthowTime, backthrowTarget, true);
        }
    }
